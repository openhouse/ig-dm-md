...
export function upsertExport(db: Database.Database, source: { sourceKind: string; sourceIdentifier: string; name: string; checksum?: string; size?: number; modifiedTime?: string; raw?: unknown }): number {
  ...
  db.prepare(`INSERT INTO source_exports (source_kind, source_identifier, name, checksum, size, modified_time, imported_at, first_imported_at, last_seen_at, last_changed_at, status, raw_metadata_json)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'imported', ?)
    ON CONFLICT(source_identifier) DO UPDATE SET
      ...
      last_changed_at=CASE
        WHEN COALESCE(source_exports.checksum, '') IS NOT COALESCE(excluded.checksum, '')
          OR COALESCE(source_exports.size, -1) IS NOT COALESCE(excluded.size, -1)
          OR COALESCE(source_exports.modified_time, '') IS NOT COALESCE(excluded.modified_time, '')
        THEN excluded.last_changed_at
        ELSE source_exports.last_changed_at
      END,
      first_imported_at=COALESCE(source_exports.first_imported_at, source_exports.imported_at, excluded.first_imported_at),
...
export function importConversations(db: Database.Database, exportId: number, conversations: NormalizedConversation[], selfNames: string[]): ImportStats {
  ...
  const tx = db.transaction(() => {
    for (const conv of conversations) {
      ...
      for (const warning of conv.warnings) if (insertWarning(db, exportId, threadId, warning)) stats.warnings++;
    ...
    db.prepare('UPDATE source_exports SET warning_count = (SELECT COUNT(*) FROM warnings WHERE source_export_id = ?) WHERE id = ?').run(exportId, exportId);
  });
  tx();
  return stats;
}

function insertWarning(db: Database.Database, exportId: number, threadId: number | undefined, warning: ParseWarning): boolean {
  const result = db.prepare('INSERT OR IGNORE INTO warnings (source_export_id, thread_id, severity, code, message, source_path, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(exportId, threadId, warning.severity, warning.code, warning.message, warning.sourcePath, new Date().toISOString());
...
export function getRenderData(db: Database.Database) {
  ...
  return {
    ...
    exports: db.prepare('SELECT * FROM source_exports ORDER BY COALESCE(first_imported_at, imported_at), id').all() as ExportRecord[],
    warnings: db.prepare(`SELECT w.*, s.name source_name, t.title thread_title FROM warnings w LEFT JOIN source_exports s ON s.id=w.source_export_id LEFT JOIN threads t ON t.id=w.thread_id ORDER BY COALESCE(s.name, ''), COALESCE(t.title, ''), w.code, w.message, COALESCE(w.source_path, ''), w.id`).all() as any[]
  };
}
